#include "vericard_animation_disco.h"
...
namespace vericard
{
   ...
   void animation_disco::_001OnDrawBefore(::draw2d::graphics * pgraphics)
   {
	...
	rShell.top = rShell.bottom;
         ...
         for (index i = 0; i < m_llama.get_count(); i++)
         {
            ...
            pgraphics->BitBlt(l.m_iOffset, rDisco.top + m_iLlamaFloor * l.m_dSize,
               rDisco.width(), rDisco.height(),
      ...
      m_dibLights->create(rLights.size());

      m_dibLights->get_graphics()->set_alpha_mode(::draw2d::alpha_mode_set);

      m_dibLights->get_graphics()->FillSolidRect(rLights, ARGB(127, 0, 0, 0));
      ...
      pgraphics->BitBlt(rectClient, m_dibLights->get_graphics());
   ...
   void animation_disco::_001OnDrawAfter(::draw2d::graphics * pgraphics)
   {
...

      {
         ...
         m_dibDisco->create(rectDisco.size());

         m_dibDisco->get_graphics()->set_alpha_mode(::draw2d::alpha_mode_set);

         //m_dibDisco->get_graphics()->FillSolidRect(0, 0, , m_dibDiscoBall->m_size.cx, m_dibDiscoBall->m_size.cy, ARGB(255, 0, 255, 0));
         ...
         m_dDiscoColor += 0.02;
         pgraphics->BitBlt(rectDisco,
            m_dibDisco->get_graphics());
