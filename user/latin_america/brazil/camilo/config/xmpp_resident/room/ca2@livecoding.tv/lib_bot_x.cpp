...
string bot_x()
{
   ...
   if (::str::begins_eat_ci(strQuery, "!anime "))
   {
      ...
      if (strAnimation.CompareNoCase("off") == 0 || strAnimation.CompareNoCase("stop") == 0)
      {
         ...
         Application.veripack().m_strAnimationStyleNew.Empty();

      }
      else
      {

         Application.veripack().m_strAnimationNew = strAnimation;
         Application.veripack().m_strAnimationStyleNew = strStyle;
   ...
   else if (::str::begins_eat_ci(strQuery, "!ss "))
   {
      ...
      call_async("C:\\core\\time\\Win32\\basis\\app_veriwell_waven.exe", "\"" + strUrl + "\" : for_resident=" + m_strUserId,  "C:\\core\\time\\Win32\\basis\\", SW_SHOW, false);
      ...
      call_async("/xcore/time/x64/basis/app", "\""+strUrl+"\" : for_resident="+ m_strUserId +" app=app-veriwell/waven build_number=basis locale=_std schema=_std ", "/xcore/time/x64/basis/", SW_SHOW, false);
      ...
      || ::str::begins_eat_ci(strText, "https://open.spotify.com/track/"))
   {
      ...
      call_async("C:\\core\\time\\Win32\\basis\\app_veriwell_waven.exe", "\"spotify:track:" + strText + "\" : play_now", "C:\\core\\time\\Win32\\basis\\", SW_SHOW, false);
      ...
      call_async("/xcore/time/x64/basis/app", "\"spotify:track:" + strText + "\" : play_now app=app-veriwell/waven build_number=basis locale=_std schema=_std ", "/xcore/time/x64/basis/", SW_SHOW, false);
   ...
   else if (strText == "!live")
   {
      ...
      sp(xmpp::comm) pcomm = m_pcomm;
      var v;
      pcomm->lctv_api_get("/api/livestreams/onair/", v, "bot");
   ...
   else if (::str::begins_eat_ci(strText, "!volume "))
   {
         ...
         call_async("C:\\core\\time\\Win32\\basis\\app_veriwell_waven.exe", " : mediaplay:set_volume=" + strText, "C:\\core\\time\\Win32\\basis\\", SW_SHOW, false);

#elif defined(__APPLE__)

         call_async("/Applications/Waven.app/Contents/MacOS/Waven", " : mediaplay:set_volume=" + strText, "/Applications/Waven.app/Contents/MacOS", SW_SHOW, false);

#else

         call_async("/xcore/time/x64/basis/app", " : app=app-veriwell/waven build_number=basis locale=_std schema=_std mediaplay:set_volume=" + strText, "/xcore/time/x64/basis", SW_SHOW, false);
         ...
         if (vara.get_size() > 1)
         {
            ...
            else
            {
               ...
               if (strLo == "next")
               {
                  ...
                  Application.veripack().broad_msg("Next Playlist: " + str);

               }
               else
               {

                  Application.veripack().broad_msg("Previous Playlist: " + str);
   ...
   else if (::str::begins_eat(strText, "!setcity "))
   {
      ...
      set_user_data(strUser, "city_set", itoa_dup(timeNow.m_time));
